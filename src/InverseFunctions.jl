...
approx_inverse(::typeof(relu)) = nothing
approx_inverse(::typeof(abs)) = nothing
###########################################################################
...
approx_inverse(f::Base.Fix1{typeof(safe_pow)}) = x -> safe_log(x) / safe_log(f.x)

# (_ ^ f.x) => _ ^ (1/f.x)
# For x^a = y, solve for x: x = y^(1/a)
